...
- Added `"connection_string"` to the default `MaskingConfig.secret_field_names`, so `AzureAppConfigSource.connection_string` is masked in error messages and logs by default.
...
- Added `AzureKeyVaultSource` (`dature[azure-keyvault]`) to load secrets from Azure Key Vault, either as a single JSON document (`name=...`) or by listing and nesting every secret in the vault.
...
- Added the missing `advanced/remote/ssm.md` and `advanced/remote/secrets_manager.md` pages, plus the two new Azure remote source pages, to the `llmstxt` plugin's file list.
...
- Updated README to list all supported sources (Consul, etcd, AWS SSM, AWS Secrets Manager) and key features (CLI, cross-source refs, conditional sources).
...
- Added ``docs/comparison/benchmarks.md`` with performance benchmarks comparing dature against
  ...
  used only for the warm-reuse table, where nothing new stays resident and it is the correct tool. ([#benchmarks](https://github.com/reagento/dature/issues/benchmarks))
...
- Several performance improvements to build and load, all behaviour-preserving — a valid config
  ...
  **EnvSource.** Values for env vars outside ``prefix`` (or, when set, outside the ``Absolute``
  ...
  ([#performance](https://github.com/reagento/dature/issues/performance))
