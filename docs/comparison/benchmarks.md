...
inside itself; one untimed sample runs first per venv to absorb `.pyc` compilation and cold page
cache, then the **min** of the remaining samples is reported (mean-of-subprocesses is dominated
by OS scheduling noise, not by the library). Speed in ms, memory as `tracemalloc` peak in MiB.

**Speed** (`bench_speed.py`) — in-process, 5 untimed warmup calls followed by
`timeit.repeat(number=500, repeat=9)`, reporting the **min** per-call time. The warmup matters:
a dature build+load's first call costs ~35× a steady-state call (adaptix's lazy init plus the
one-time `DATURE_` config bootstrap), so without it the first `timeit` repeat silently absorbs
that cost. `min` rather than `mean` because a mean over a handful of repeats on a shared machine
tracks scheduler jitter, not the code — `timeit`'s own docs recommend `min` for this reason. Every
library re-declares its model class each call, so it's apples-to-apples.
...
|---------|-------------:|---:|--------------:|---:|
| adaptix (dature's engine) | 73.8 ms | baseline | 8.3 MiB | baseline |
| pydantic-settings | 73.9 ms | baseline | 12.3 MiB | 1.5× |
| dature | 129.9 ms | 1.8× | 13.1 MiB | 1.6× |

dature imports in ~130 ms and ~13.1 MiB — about 1.8× pydantic-settings on time, and close on
memory. Most of dature's import is adaptix (its type engine, ~74 ms); dature's own code (17
source modules, each pulling in its slice of `dature.coercion`/`dature.errors`/adaptix
providers) adds ~56 ms. This is a one-time cost paid once per process, not per config load.
...
|---------|------:|---:|-------------:|---:|
| python-decouple | 161 µs | baseline | 11.2 KiB | baseline |
| pydantic-settings | 309 µs | 1.9× | 27.3 KiB | 2.4× |
| dature (func) | 1.2 ms | 7.6× | 22.5 KiB | 2.0× |
| dature (decorator, `cache_engine=False`, default) | 1.3 ms | 8.1× | 30.8 KiB | 2.8× |
| dature (decorator, `cache_engine=True`) | 1.3 ms | 8.2× | 102.3 KiB | 9.1× |
| dynaconf | 3.7 ms | 22.8× | 11.7 KiB | baseline |

### ENV file (`.env` → typed dataclass)

| Library | Speed | vs | Memory (RSS) | vs |
|---------|------:|---:|-------------:|---:|
| python-decouple | 345 µs | baseline | 11.2 KiB | baseline |
| pydantic-settings | 918 µs | 2.7× | 28.0 KiB | 2.5× |
| dature (decorator) | 2.4 ms | 7.0× | 31.6 KiB | 2.8× |
| dature (func) | 2.6 ms | 7.5× | 22.5 KiB | 2.0× |
| dynaconf | 9.8 ms | 28.3× | 12.9 KiB | 1.1× |

### JSON file

| Library | Speed | vs | Memory (RSS) | vs |
|---------|------:|---:|-------------:|---:|
| pydantic-settings | 529 µs | baseline | 28.1 KiB | 2.4× |
| dature (func) | 2.4 ms | 4.5× | 23.4 KiB | 2.0× |
| dature (decorator) | 2.4 ms | 4.6× | 32.6 KiB | 2.8× |
| dynaconf | 3.2 ms | 6.0× | 11.5 KiB | baseline |
...
| Library | Speed | vs | Memory (RSS) | vs |
|---------|------:|---:|-------------:|---:|
| pydantic-settings | 520 µs | baseline | 28.2 KiB | 2.5× |
| dature (func) | 2.5 ms | 4.7× | 23.1 KiB | 2.0× |
| dature (decorator) | 3.1 ms | 6.0× | 32.4 KiB | 2.8× |
| dynaconf | 3.7 ms | 7.1× | 11.4 KiB | baseline |
...
| Library | Speed | vs | Memory (RSS) | vs |
|---------|------:|---:|-------------:|---:|
| pydantic-settings | 618 µs | baseline | 28.2 KiB | 2.4× |
| dature (func) | 3.0 ms | 4.9× | 24.0 KiB | 2.0× |
| dature (decorator) | 3.2 ms | 5.2× | 32.7 KiB | 2.8× |
| dynaconf | 3.5 ms | 5.7× | 11.8 KiB | baseline |
| hydra (DictConfig, not typed) | 20.7 ms | 33.4× | — | — |
...
| Library | Speed | vs | Memory (RSS) | vs |
|---------|------:|---:|-------------:|---:|
| pydantic-settings | 677 µs | baseline | 70.4 KiB | 1.1× |
| dature (func) | 2.5 ms | 3.7× | 64.8 KiB | baseline |
| dature (decorator) | 2.6 ms | 3.8× | 73.5 KiB | 1.1× |
...
| Library | Speed | vs | Memory (RSS) | vs |
|---------|------:|---:|-------------:|---:|
| pydantic-settings | 599 µs | baseline | 45.3 KiB | 4.8× |
| dature (func) | 2.6 ms | 4.4× | 41.3 KiB | 4.4× |
| dature (decorator) | 2.7 ms | 4.5× | 67.1 KiB | 7.1× |
| dynaconf | 6.4 ms | 10.7× | 9.5 KiB | baseline |
...
**memory** dature (func) retains ~22–65 KiB per build — the string-value sources (ENV, ENV file)
sit slightly lower than the file-format sources (JSON, TOML, YAML, ~23–24 KiB). dature (decorator)
retains more (~100 KiB in the ENV table) because a decorated class **keeps its `Loader` and
compiled retort alive** for the class lifetime — a one-time cost per class, not per load, and
exactly what makes warm reuse cheap (see below).
...
|------|------:|---:|-------:|---:|
| dature — decorator, hot, `cache=True` (default) | 0.5 µs | baseline | 0.9 KiB | baseline |
| dature — decorator, hot, `cache=timedelta(...)` (TTL) | 0.6 µs | 1.2× | 1.0 KiB | baseline |
| dature — `Loader` reuse, `cache_engine=True` | 73 µs | 140× | 10.4 KiB | 11.0× |
| dature — decorator, hot, `cache_engine=True` | 76 µs | 146× | 10.6 KiB | 11.2× |
| pydantic-settings — reuse | 129 µs | 248× | 25.6 KiB | 27.0× |
| dature — decorator, hot, `cache_engine=False` | 884 µs | 1696× | 437.0 KiB | 460.8× |
| dature — function mode, fixed schema, no reuse | 1.0 ms | 2007× | 438.8 KiB | 462.7× |

In steady state, opting into `cache_engine=True` **flips** the ranking versus build+load: dature
(`Loader` reuse) is **faster** than pydantic-settings reused (~73 µs vs ~129 µs) and about half
the memory (10.4 vs 25.6 KiB). Caching (`cache=True`/`cache=timedelta`) drops dature to ~0.5 µs /
~1 KiB — over 200× faster than either. `cache=timedelta` adds automatic TTL expiry a plain
...
`cache=False`) and function mode pay full recompilation every time — ~884 µs–1.0 ms and ~438 KiB of
...
pieces are independent: import (~130 ms, once per process), fresh build+load (~1.2–3.2 ms, only
in function mode), and warm reuse (~73 µs with `cache_engine=True`, ~0.5 µs cached).

**Import is reasonable — comparable to pydantic-settings, close on RAM.** ~130 ms vs ~74 ms,
and 13.1 MiB vs 12.3 MiB. (A native measurement inside a fat venv reports ~2× higher for everyone —
...
call costs ~1.2–3.2 ms. Even with a fixed schema, a throwaway `Loader` per call costs ~1.0 ms
because each fresh loader discards its setup — build the loader once *and* opt into
`cache_engine=True` (decorator or `Loader` reuse) and it drops to ~73–76 µs; cache the result
(`cache=True`, the default) and it's ~0.5 µs. Function mode is for scripts and one-shot tools, not
per-request loading.

**`cache_engine` controls whether the compiled engine is retained — default off.** At the default
`cache_engine=False`, dature (decorator) retains only ~31 KiB per build, close to dature (func)
...
exchange for a ~12× faster hot path (~76 µs vs ~884 µs) — the right call for a long-running
...
native-aware metric), dature (func) holds ~22–24 KiB per build vs pydantic's ~27–28 KiB.
...
`cache_engine=True`, dature is faster (~73 µs vs ~129 µs) and about half the memory, and cached
it's over 200× faster than either. For a long-running service — build once, load many — that
